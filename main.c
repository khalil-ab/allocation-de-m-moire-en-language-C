...
{
    char code[10];
    char etat_alloc;//si le processus est alloué ou non
    char etat_attente_mem;//si le processus est en attente de la memoire ou non
    int date_arr;
    int tmp_exec;
    int esp_mem;
    ...
    else
    {
        ...
        while(fgets(ch,sizeof(ch),p)!=NULL)
        {
            if(strcmp(ch,"\n")!=0)
                ...
                {
                    printf("Votre fichier contient un saut de ligne qu'il faut %cviter !!",130);
                    getch();
                    return 0;
                }
        }
...
int verif_nombre(char nb[],int max)
{
    ...
    if(erreur==0)
    {
        taille=atoi(nb);
            ...
            {
                printf("\nDonnez une taille non nulle de minimum %d Ko !!",max);
                return 0;
            }
            else
                return atoi(nb);
...
void inserer_cmt(liste_cmt *comentaire,int instant,char *code,int debut_partition,int fin_partition,int dur,bloc_mem tete,processus *first,int type)
{
    ...
    CharToOemBuff("à", a, sizeof a - 1);
    CharToOemBuff("é", e, sizeof e - 1);
    CharToOemBuff("ê", ee, sizeof ee - 1);
    ...
    if(type==1)
    {
       ...
       for(q=msg;*q!='\0';q++);
           ...
           for(q=msg;*q!='\0';q++);
            ...
            for(q=msg;*q!='\0';q++);
            strcpy(q," dans la partiton [");

            sprintf(conv, "%d", debut_partition); // Conversion de l'entier
            for(q=msg;*q!='\0';q++);
            strcpy(q,conv);
          ...
          sprintf(conv, "%d", fin_partition); // Conversion de l'entier
            for(q=msg;*q!='\0';q++);
            strcpy(q,conv);
    ...
    else
    {
       ...
       for(q=msg;*q!='\0';q++);
           ...
           for(q=msg;*q!='\0';q++);
            ...
            sprintf(conv, "%d",dur+instant); // Conversion de l'entier
            for(q=msg;*q!='\0';q++);
            strcpy(q,conv);
...
processus * chercher_proch_proc_a_executer_SJF(liste_processus *LC_prc,liste_bloc_mem *LC_bm,liste_cmt*comentaire,int instant)
{
    ...
    for(proc=LC_prc->tete; proc!=NULL; proc=proc->suiv) //chercher la plus courte duree d execution
    {
    ...
    for(proc=LC_prc->tete; proc!=NULL; proc=proc->suiv)//verifier s il y a un autre processus ayant la meme courte duree d execution et en meme temps pointer sur le processus a executer
    {
    ...
    for(bm=LC_bm->tete; bm!=NULL; bm=bm->suiv)//donner l etat elu au processus a exucuter
        {
...
void inserer_ds_RAM(processus *pr,bloc_mem *bm,liste_bloc_mem *mem,int instant,liste_cmt *comentaire,liste_processus *lp)
{
    ...
    if(pr->esp_mem==bm->taille)//inserer dans la case meme
    {
    ...
    else
    {
        ...
        if(bm->prec!=NULL)//inserer au milieu
        {
            nv_bm->suiv=bm;
            nv_bm->prec=bm->prec;
            ...
            bm=nv_bm;//pour faire l insertion du commentaire
        }
        else//inserer à la tete
        {
            nv_bm->suiv=bm;
            nv_bm->prec=NULL;
            strcpy(nv_bm->code_proc,pr->code);
...
{
    if(proc->prec!=NULL&&proc->suiv!=NULL)//le processus a liberer est situé au milieu de la LC des processus
    {
    ...
    {
        if(proc->prec==NULL&&proc->suiv!=NULL)//le processus a liberer est situéà la tete de la LC des processus
        {
        lp->tete=proc->suiv;
        proc->suiv->prec=NULL;
        free(proc);
        ...
        {
             if(proc->prec!=NULL&&proc->suiv==NULL)//le processus a liberer est situéà la fin de la LC des processus
             {
                lp->queue=proc->prec;
                proc->prec->suiv=NULL;
                free(proc);
             }
             else//le processus a liberer est le seul qui reste dans la LC des processus
             {
                lp->queue=NULL;
                lp->tete=NULL;
                free(proc);
...
{
    bloc_mem *BM,*q;
    for(BM=lbm->tete; BM!=NULL; BM=BM->suiv) //chercher le processus alloué dans la memoire
    {
    ...
    if(BM->prec==NULL&&BM->suiv!=NULL)//le processus a liberer est a la tete de la liste des BM
    {
    ...
    {
        if(BM->prec!=NULL&&BM->suiv!=NULL)//le processus a liberer est au milieu de la liste des BM
        {
        ...
        {
            if(BM->prec!=NULL&&BM->suiv==NULL)//le processus a liberer est à la fin de la liste des BM
            {
            ...
            else//le processus a liberer est le seul qui occupe toute la memoire
            {
                BM->etat_bloc='l';
                lbm->nb_proc_a_executer--;
            }
...
int allouer_fifo(liste_processus *lp,int instant,liste_bloc_mem *mem,int choix,liste_cmt *cmt)
{
    ...
    while(proc!=NULL)
    {
        if(proc->date_arr<=instant&&proc->etat_alloc=='n')//date d'arrivée et si le proc est deja alloué(o=oui) dans la memoire ou pas (n=non)
        {
            if(choix==2)
            ...
            if(adr!=NULL)
            {
                ...
                inserer_ds_RAM(proc,adr,mem,instant,cmt,lp);//inserer tel processus à l'adresse adr de la memoire mem
...
void titre(void)
{
    printf("                         -----------------------------------------------------\n");
    printf("                        |                   ~~ A2M BF-FF ~~                   |\n");
    printf("                        |           PROGRAMME D'ALLOCATION DE MEMOIRE         |\n");
    printf("                         -----------------------------------------------------\n");
}

void introduction(void)
{
   printf("\n\n\n\t\t\t\t       Salutation (^_^) !!\n\t Voici un programme de gestion de m%cmoire  selon le type d'allocation choisis\n ",130);
    printf(" sachant que la file d'attente de m%cmoire est g%cr%ce par FIFO et celle du processeur est g%cr%ce par SJF ",130,130,130,130,130);
...
int menu_ch_algo(void)
{
    ...
    printf("\n\t\t                    --------------------------------\n");
    printf("\t\t                   |    ALGORITHMES D'ALLOCATION    |\n");
    printf("\t\t                    --------------------------------\n");
    printf("\t\t                   |          1. FIRST FIT          |\n");
    printf("\t\t                   |          2. BEST  FIT          |\n");
    printf("\t\t                    --------------------------------\n");
                ...
                else
                {
                    if(rep==51)
                    {
                        ...
                        do
                        {
                            ...
                            {
                                maj=allouer_fifo(&LC_prc,instant,&LC_bm,choix,&comentaire);//la fonction envoie 1 dans 2 cas : allocation d'un processus ou attente d'un processus soit 0 s'il n y a pas de changement a effectuer
                            }
                    ...
                    printf("\n\nR : retourner au menu principal\t\t\t\t\t    M : retourner au menu choix des algorithmes");
                    do
                    {
                        rep=getch();
                    }
